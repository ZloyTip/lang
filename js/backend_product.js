if(!$.product.editTabLoadContent_lang_replaced) {
$.product.editTabLoadContent_lang_replaced = true;
var langPluginOriginalEditTabLoadContent = $.product.editTabLoadContent;
$.product.editTabLoadContent = function(path, post) {
    if (path.tab == 'lang') {
        var self = this;
        path = path || this.path;
        var url = '?plugin=lang&module=product&action=settings&id=' + path.id;
        var r = Math.random();
        this.ajax.random = r;
        ...
        return langPluginOriginalEditTabLoadContent.apply(this, arguments);
    }
};
}
